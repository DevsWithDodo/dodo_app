...
class RecommendedPayments extends StatefulWidget {
  ...
  const RecommendedPayments(
      {super.key, required this.members, required this.onChange, this.onlyShowOwn = true});
...
class _RecommendedPaymentsState extends State<RecommendedPayments> {
  ...
  void initState() {
    ...
    if (widget.onlyShowOwn) {
      _necessaryPayments = payments.where((payment) => payment.payerId == context.read<AppStateProvider>().user!.id).toList();
    } else {
